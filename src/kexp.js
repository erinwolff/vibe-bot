...
module.exports = function kexpCommand() {
  async function handleKEXPCommand(interaction) {
    ...
    try {
      ...
      return interaction.editReply("**Now streaming KEXP Radio live!** Enjoy the tunes ⋆♫˚.⋆⭒.˚⋆");
    } catch (error) {
      console.error("Error in KEXP command:", error);
      
      // Return a shorter error message
      return interaction.editReply({
        content: "Failed to play KEXP radio. Please check the console for more details.",
        ephemeral: true
      }).catch(err => {
